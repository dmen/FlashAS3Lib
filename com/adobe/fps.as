	...
	*/	
	...
	public dynamic class fps 
	{
		...
		public static var services:Object = { instanceData: { }, 
			_LoaderProxy: { 
				getLoader:fps.getLoader, 
				getURLRequest:fps.getURLRequest,
				loadRequest:fps.loadRequest
			}
		};
			...
			try { 
				if ( 0 == attemptNumber ) {								
					if ( null == mcRoot || null == mcRoot.stage ) {					
						flash.utils.setTimeout(fps.wait4Stage, 500, conf, params, mcRoot,sServices,callback,attemptNumber + 1);					
						return;
					}					
				}
				else {					
				...
				fps.loadWithStage(conf, params, mcRoot, sServices, callback);
			}
			catch (e:Error) {
				trace('fps.as: wait4Stage attemptNumber=' + attemptNumber + ' err=' + e.message);
				flash.utils.setTimeout(fps.wait4Stage, 500, conf, params, mcRoot,sServices,callback,attemptNumber + 1);					
			}
		}
					
		public static function load(conf:Object, params:Object):void {
			Security.allowDomain('cdn.gigya.com', 'wildfire.gigya.com', 'office.fps.com');
			...
			fps.wait4Stage(conf,params,mcRoot,sServices,callback,0);
		}
		...
		public static function isLoaded(conf:Object,params:Object,serviceName:String):Boolean  {				
			var currentServiceObj:* = fps.services[serviceName];
		...
		private static function CreateServiceLoader( mcRoot:Sprite, sService:String , serviceListener:Object, tasks:Array):Object {
			...
			task['IO_ERROR'] = function(evt:IOErrorEvent):void {
				trace('fps.as: IOErrorEvent called while trying to load '+task['serviceName'] ); 
			...
			task['COMPLETE'] = function(event:Event):void {
				...
				var serviceInstance:Object = new serviceClass(); // to trigger allowDomain && allow access from the service to fps.*				
				fps.services[serviceName] = serviceClass;
				if (serviceClass['setGigyaClassFromParent']) serviceClass['setGigyaClassFromParent'](ApplicationDomain.currentDomain.getDefinition('com.adobe.fps'));				
				task['serviceListener'].onService(false,ithTask['serviceName'],'');

			};

			task['fps'] = fps;
			task['services'] = fps.services;
