...
package com.gmrmarketing.sap.nhl2015.gda.cloud
{
	...
	{		
		...
		private var tags:Array; //the tags returned by the service - set in tagsLoaded()
		 ...
		 * @param	colors
		 */
		public function refreshTags(colors:Array):void
		{	
			tagColors = colors;
			getTagsByLevel();
		}
		...
		{			
			...
			var r:URLRequest = new URLRequest("http://sap49ersapi.thesocialtab.net/api/NHL/GetCachedFeed?feed=NHLWordCloud");					
		...
		{	
			//data is an array of objects with tag and count keys			
			...
			for (var i:int = 0; i < tags.length; i++) {				
				...
				tags[i].count = Math.log(tags[i].count);				
				totalTagCount += tags[i].count;
				
				var name:String = tags[i].tag;
				name = name.replace(/&lt;/g, "<");
				name = name.replace(/&gt;/g, "<");
				name = name.replace(/&amp;/g, "&");
