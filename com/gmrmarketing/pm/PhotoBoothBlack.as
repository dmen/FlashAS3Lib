...
{	
	...
	public class PhotoBoothBlack extends MovieClip
	{
		...
		private var dashboardData:BitmapData;
		...
		private var textShadow:DropShadowFilter;
		...
		public function PhotoBoothBlack()
		{			
		...
		private function init():void
		{
			...
			thanks.x = 776;
			thanks.y = 416;
			thanks.filters = [textShadow];
			
			choose = new choosebw(); //lib clip
			choose.x = 978;
			choose.y = 534;
			choose.filters = [textShadow];
			...
			try{
				l.load(new URLRequest("config.xml")); //contains path for rfid check service
			}catch (e:Error) {
				quitPhotoBooth();
			}
		}
		...
		/**
		 * Called from configLoaded - adds the take pic button and the camera video to the stage 
		 * called from reset()
		 */
		...
		{	
			...
			var fm2:Matrix = new Matrix();
			fm2.scale(525 / 625, 525 / 623);
			
			fullSize.draw(camPic.getCapture(), fm, null, null, null, true);
			fullSize.draw(dashboardData, fm2, null, null, null, true);
			
			fullSize2.draw(camPic.getCapture(), fm, null, null, null, true);
			fullSize2.draw(dashboardData, fm2, null, null, null, true);
			...
			m3.scale(196 / 625, 206 / 623);
			
			thumb.draw(camPic.getCapture(), m2, null, null, null, true);
			thumb.draw(dashboardData, m3, null, null, null, true); //draw the dashboard overlay
			
			grayThumb.draw(camPic.getCapture(), m2, null, null, null, true);
			grayThumb.draw(dashboardData, m3, null, null, null, true); //draw the dashboard overlay
		...
		{	
			...
			TweenMax.to(grayFilmStrip, 1, { x:349, y:290, rotation:-5, delay:.3, ease:Bounce.easeOut } );
			//grayFilmStrip.x = 525;
			//grayFilmStrip.y = 150;
			//grayFilmStrip.rotation = 7.475;
			
			TweenMax.to(filmStrip, 1, { x:1351, y:297, rotation: -4, delay:.3, ease:Bounce.easeOut } );
			//filmStrip.x = 1162;
			//filmStrip.y = 160;
