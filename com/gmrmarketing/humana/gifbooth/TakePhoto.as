...
{	
	...
	public class TakePhoto extends EventDispatcher
	{
		...
		private var numSeconds:int;//thse are set in show()
		...
		/**
		 * Params from ConfigDialog
		 * @param	seconds
		 * @param	nth
		 * @param	totFrames
		 */
		public function show(seconds:int, nth:int, totFrames:int):void
		{
			trace("tajkePhoto.show", seconds, nth, totFrames);
			numSeconds = seconds;
			everyNth = nth;
			maxFrames = totFrames;
		...
		*/
		...
		private function grabFrame(e:Event):void
		{
			...
			Utility.drawArc(arcContainer.graphics, 0, 0, 98, 0, 360 * (frames.length / maxFrames), 25, 0xffffff, 1);
		}
