...
package com.gmrmarketing.website
{
	...
	{		
		...
		private var categories:Array = new Array("sports", "entertainment", "lifestyle", "retail", "digital", "consulting", "mobile", "corporate");
		...
		public function Main5()
		{
			...
			addChild(practiceLoader);
			practiceLoader.y = -5000;
		 ...
		 * 
		 * This is the menu of people images
		 */
		...
		{	 
			...
			for (i = 0; i < xMenu.length(); i++) {
				...
				var ind:int; //display list index
				...
				iconBtn.alpha = 0;
				iconBtn.ind = ind;
				iconMenu.addChildAt(iconBtn,ind);
		...
		private function enableMenu():void
		{
			...
			var bt:MovieClip;
			for (i = 0; i < c; i++) {
				bt = MovieClip(navigation.getChildAt(i));
				bt.buttonMode = true;
				bt.addEventListener(MouseEvent.CLICK, menuClick, false, 0, true);
				bt.addEventListener(MouseEvent.MOUSE_OVER, menuHilite, false, 0, true);
				bt.addEventListener(MouseEvent.MOUSE_OUT, menuUnHilite, false, 0, true);
			}
			c = iconMenu.numChildren;
			
			//icon menu - the little people
			for (i = 0; i < c; i++) {
				bt = MovieClip(iconMenu.getChildAt(i));
				bt.buttonMode = true;
				bt.addEventListener(MouseEvent.CLICK, menuClick, false, 0, true);
				bt.addEventListener(MouseEvent.CLICK, iconUnHilite, false, 0, true);
				bt.addEventListener(MouseEvent.MOUSE_OVER, iconHilite, false, 0, true);
				bt.addEventListener(MouseEvent.MOUSE_OUT, iconUnHilite, false, 0, true);
			}
		...
		{		
			iconMenu.setChildIndex(MovieClip(e.currentTarget), iconMenu.numChildren - 1);
		...
		{
			//if(e.currentTarget.cat != categoryClicked){
			iconMenu.setChildIndex(MovieClip(e.currentTarget), MovieClip(e.currentTarget).ind);
		...
		{		
			...
			if (categoryClicked == -1) {
				...
				//practiceLoader.unload();
				//practiceLoader = null;
				//showIntroElements();
				//trace("neg 1");
			}else {
				//MENU ITEM OR ICON MENU ITEM CLICKED
				//removeIntroElements();
				navigateToURL(new URLRequest("practices.aspx#/" + categories[categoryClicked]), "_self");
				//loadPractices();
			}
		}
		...
		/**
		 * Shows the navigation menu under the logo
		 * this is the list of text buttons
		 */
		...
		private function showIntroElements():void
		{
			...
			var l:int;
			var j:int;
			l = iconMenu.numChildren;
			for (j = 0; j < l; j++) {
				iconMenu.getChildAt(j).alpha = 0;
			}
			...
			TweenLite.to(iconMenu.getChildAt(l-1), .5, { alpha:1, delay:j * .25, onComplete:enableMenu } );
		...
		{		
			if(practiceLoader){
				if (contains(practiceLoader)) {				
					practiceLoader.y = -5000;// removeChild(practiceLoader);
				}
			}
			if(introCopy != null){
		...
		private function autoScrollOn():void
		{			
			
			scrollTimer.addEventListener(TimerEvent.TIMER, nextScroll, false, 0, true);
			scrollTimer.reset();
			scrollTimer.start();			
		}
		
		
		private function autoScrollOff():void
		{
			scrollTimer.stop();			
		}
		
		
		private function nextScroll(e:TimerEvent):void
		{
			if (reachedBottom) {
		...
		private function dnClicked(e:MouseEvent = null):void
		{			
			...
			var scrollAmt:int = 240;
			if (e == null) {
				//called from nextScroll()
				scrollAmt = 120;
			}
			
			if (thumbsBottom >= maskBottom + scrollAmt) {
				TweenLite.to(thumbs, 1.5, { y:thumbs.y - scrollAmt } );				
			}else {
		...
		private function showVidInfo(e:MouseEvent):void
		{
			...
			TweenLite.to(detail, .5, { scaleX:1, scaleY:1, alpha:1, onComplete:beginMouseChecking } );			
			...
			detail.addEventListener(MouseEvent.CLICK, playVid, false, 0, true);
			detail.buttonMode = true;
			detail.theCategory.mouseEnabled = false;
			detail.theTitle.mouseEnabled = false;
			detail.theDescription.mouseEnabled = false;
			detail.btnWatch.mouseEnabled = false;
			detail.btnWatch.theText.mouseEnabled = false;
			
			var thisVid:XML = introXML.videos.video[lastVideo.videoIndex];
			detail.theCategory.htmlText = thisVid.category;
			detail.theTitle.htmlText = thisVid.title;
			detail.theDescription.htmlText = thisVid.description;
			detail.btnWatch.theText.htmlText = thisVid.buttonText;
		...
		private function beginMouseChecking():void
		{
			addEventListener(Event.ENTER_FRAME, checkMouseDetail, false, 0, true);
		}
		 ...
		 */
		private function checkMouseDetail(e:Event):void
		{
		...
		{			
			//trace("hide vid info");
			removeEventListener(Event.ENTER_FRAME, checkMouseDetail);
		...
		/**
		 * Called from clicking the watch button on the video detail
		 * @param	e
		 */
		private function playVid(e:MouseEvent):void
		{
			...
			if(lastVideo.link == "vid"){
				...
				//trace(lastVideo.target);
				navigateToURL(new URLRequest(lastVideo.vid), lastVideo.target);
			}
			hideVidInfo();
		}
		...
		{			
			...
			bigVideo.y = 60;// Math.max(50, (stage.stageHeight - infoObject.height) * .5);
