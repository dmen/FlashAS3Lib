 ...
 * 
 * THIS VERSION - 3 - IS FOR THE THIRD COLUMN WITH VIDEO LINK
 * 
 * 
 * 
 * 
...
package com.gmrmarketing.website
{
	...
	{		
		...
		private var introHead:*;
		private var introCopy:*;
		...
		private var showIgnition:Boolean = false;
		...
		public function Main3()
		{
			...
			closeButton = new closeBtn(); //lib clips
			...
			var req:URLRequest = new URLRequest(basePath + "intro.xml");//contains menu list and video list for ignition sessions
		...
		private function introXMLLoaded(e:Event):void
		{
			...
			showIgnition = introXML.videos.video.length() == 0 ? false : true;
			if (showIgnition) {
				introHead = new introHeadlineIG(); //use shorter text fields when videos on screen
				introCopy = new introBodyIG();
			}else {
		...
		{			
			...
			if (!swfAddressMenuChange) {
				...
				if (cat == -1) {
					...
					//tracer.doTrace("swfaddress playcube");
				}else {
		...
		{
			removeNav(); //removes logo button and navigation
			playCube(5);
			if(showIgnition){
				TweenLite.to(logoVideo, 3, { x: -90, y:0, width:624, height:624, onComplete:showIntroNavigation } );
			}else {
		...
		{
			if(showIgnition){
				TweenLite.to(logoVideo, .75, { x: -90, onComplete:showIntroNavigation } );
			}else {
		...
		private function showIntroNavigation():void
		{
			...
			if(language == "en"){
				...
				playVid.x = (logoVideo.x + logoVideo.width / 2) - 20;
				playVid.y = logoVideo.y + 350;
			...
			*/
			...
			for (var i:int = 0; i < numVids; i++) {
				...
				vid.theDescription.autoSize = TextFieldAutoSize.LEFT;
				vid.theDescription.text = thisVid.description;
				var textY:Number = 180 - vid.theDescription.textHeight - 10;
				vid.theDescription.y = textY;
				vid.theDescription.mouseEnabled = false;
				vid.theDescription.alpha = 0;
				vid.bg.y = textY;
				vid.bg.height = 180 - textY;
				vid.bg.alpha = 0;
				...
				vid.addEventListener(MouseEvent.MOUSE_OUT, hideVidInfo, false, 0, true);
				vid.addEventListener(MouseEvent.CLICK, playVid, false, 0, true);
				vid.buttonMode = true;
				...
				iLoader.x = 3;
				iLoader.y = 30;
				var play:playRing = new playRing(); //lib clip - circle with play button
				vid.addChild(play);
				play.x = 142;
				play.y = 95;
			}
		...
		{
			TweenLite.to(e.currentTarget.bg, .5, { alpha:.7 } );
			TweenLite.to(e.currentTarget.theDescription, .5, { alpha:1 } );	
		}
		
		private function hideVidInfo(e:MouseEvent):void
		{
			TweenLite.to(e.currentTarget.bg, .5, { alpha:0 } );
			TweenLite.to(e.currentTarget.theDescription, .5, { alpha:0 } );			
		}
		
		private function playVid(e:MouseEvent):void
		{
			if(e.currentTarget.link == "vid"){
				selectedVideo = e.currentTarget.vid;
				startMainVideo();
			}else {
				navigateToURL(new URLRequest(e.currentTarget.vid), "_self");
			}
		}
		...
		{			
			...
			if (contains(vidContainer)) { TweenLite.to(vidContainer, .75, { alpha:0 } ) }
			TweenLite.to(introCopy, .75, { alpha:0 } );
		...
		{		
			...
			if (whichVid == 5) {
				...
				if (logoVideo.x > -160) {
					//video of cube 4 is still in the opening nav position - it was never docked					
					logoVideo.width = 448;
					logoVideo.height = 454;
					logoVideo.x = -168;
					logoVideo.y = -150;
					logoVideo.alpha = 0;						
				}
		...
		private function showNavElements():void
		{
			...
			TweenLite.to(logoVideo, .5, { alpha:1 } );
		...
		{ 			
			...
			//tracer.doTrace("statusHandler:" + e.info.code);
