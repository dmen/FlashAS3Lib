...
package com.gmrmarketing.website
{
	...
	{		
		...
		public function Main()
		{
			...
			//to show cube video after 300ms
			logoTimer = new Timer(300, 1);
			logoTimer.addEventListener(TimerEvent.TIMER, showLogo, false, 0, true);
		}
		...
		private function dockNav():void
		{
			...
			//logoVideo.width = 545; logoVideo.height = 530; logoVideo.x = -218; logoVideo.y = -188;
			...
			if(!isNavDocked){
				...
				TweenLite.to(navigation, 1, { y:145, alpha:1, delay:.5 } );
		...
		private function removeUndockedNav()
		{
			removeNav();
			playCube(5,1);
			TweenLite.to(logoVideo, 3, { x:0, y:0, width:618, height:618, ease:Cubic.easeOut, onComplete:showIntroNavigation } );		
		}
		...
		{			
			...
			TweenLite.to(logoVideo, 2.8, { width:545, height:530, x:-218, y:-188, delay:1.2, ease:Cubic.easeIn, onComplete:loadPractices} );
		}
		...
		/**
		 * Called from constructor after 300ms
		 * @param	e
		 */
		private function showLogo(e:TimerEvent):void
		{
			logoTimer.reset();
			logoVideo.alpha = 1;
		}
