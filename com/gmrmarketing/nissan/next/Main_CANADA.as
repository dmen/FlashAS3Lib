package com.gmrmarketing.nissan.next
{
	...
	import com.gmrmarketing.nissan.next.RFID_CANADA;
	import com.gmrmarketing.nissan.next.XMLLoader;
	import com.gmrmarketing.nissan.next.Welcome_CANADA;
	import com.gmrmarketing.nissan.next.Nav;
	import com.gmrmarketing.nissan.next.FleetViewer_CANADA;
	import com.gmrmarketing.nissan.next.Cool_CANADA;
	import com.gmrmarketing.nissan.next.CoolEntry_CANADA;
	...
	public class Main_CANADA extends MovieClip
	{
		private var contentContainer:Sprite;
		
		private var xmlLoader:XMLLoader;
		private var clouds:Clouds;
		private var rfid:RFID_CANADA;
		private var welcome:Welcome_CANADA;
		private var fleetViewer:FleetViewer_CANADA;
		private var cool:Cool_CANADA;
		private var coolEntry:CoolEntry_CANADA;
		...
		public function Main_CANADA()
		{
			...
			reset.init(this, "lr");
			reset.customLoc(1, new Point(1216, 618));
		...
		private function xmlLoaded(e:Event):void
		{
			...
			rfid = new RFID_CANADA();			
			rfid.addEventListener(RFID.CHECK_GOOD, goodID, false, 0, true);
			
			nav = new Nav(this);
			cool = new Cool_CANADA(xmlLoader.getApprovedPostsURL());
			
			coolEntry = new CoolEntry_CANADA(xmlLoader.getPostMessageURL());
			coolEntry.addEventListener(CoolEntry_CANADA.ENTRY_SUBMITTING, showCoolSubmitting, false, 0, true);			
			coolEntry.addEventListener(CoolEntry_CANADA.ENTRY_SUBMITTED, showCoolSubmitted, false, 0, true);
			
			fleetViewer = new FleetViewer_CANADA(xmlLoader.getFleetXML(), 140, -40);
			fleetViewer.addEventListener(FleetViewer_CANADA.NEW_CAR_PICKED, showModelDetail, false, 0, true);
		...
		private function navSelection(e:Event):void
		{
			...
			switch(sel) {
				...
				case "cool":
					...
					break;
				case "coolEntry":					
					coolEntry.show(contentContainer, "", "");
					nav.disabeCreateOne();
					nav.disableSubmit();
		 ...
		 * @param	e CoolEntry_CANADA.ENTRY_SUBMITTED
		 */
