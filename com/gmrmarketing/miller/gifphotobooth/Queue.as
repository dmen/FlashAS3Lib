...
package  com.gmrmarketing.miller.gifphotobooth
{
	...
	public class Queue extends EventDispatcher  
	{
		...
		public function Queue()
		{
			...
			hubble.addEventListener(Hubble.FOLLOWUP_ERROR, hubblePhotoError);
			hubble.getToken();
		}
		...
		{			
			...
			log.log("Queue.add: " + data.email + " | " + data.phone + " | " + "deviceResponseID: " + data.deviceResponseID);
		...
		private function uploadNext():void
		{
			if (hubble.hasToken() && !hubble.isBusy() && users.length > 0) {
				...
				hubble.submit(new Array(curUpload.dob, curUpload.email, curUpload.phone, gString, curUpload.opt1, curUpload.opt2, curUpload.opt3, curUpload.opt4, curUpload.opt5, curUpload.deviceResponseID, curUpload.responseID));
		...
		{			
			log.log("Queue.hubblePhotoError()"+" responseId:"+hubble.responseID);
