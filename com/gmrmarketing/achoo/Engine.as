...
{	
	...
	public class Engine extends Sprite
	{
		...
		public static const GAME_WIDTH:uint = 1081; //static constants used by other classes for positioning
		public static const GAME_HEIGHT:uint = 768;
		public static const FULL_WIDTH:uint = 1360; //full width - used for centering the high scores dialog
		...
		private const BUGS_PER_LEVEL:uint = 25; //how many bugs released before upping the level
		...
		private var maxMisses:uint = 36; //misses before sneeze occurs - change incMisses() if this is changed
		...
		/**
		 * Called from intro - makeEngine()
		 * 
		 * Sets game variables, adds background graphic per room type
		 * 
		 * @param	theRoom String - bathroom, bedroom or classroom
		 */
		...
		public function setLevel(newLev:uint = 1):void
		{
			...
			theLevel = newLev;			
			bugChance = (theLevel + 1) * .03; //add 1 to make initial level harder
			theHUD.showLevel(theLevel);
		...
		*/
		...
		{		
			...
			helper.log("Game Over - Sneeze");
			
			//play final sneeze sound
			var channel:SoundChannel = s.play();
			
			
			myTimer = new Timer(PUMP_ON_TIME, 1);
			...
			helper.attractStop(); //stop attract loop checking during final messages
		...
		{
			helper.attractStart(); //resume attract loop checking
		...
		/**
		 * Turns on the sneeze relay by calling the
		 * relayON function within the SWFKit project
		 */
		private function turnOn():void
		{
			//helper.log("Sneeze Relay On");
			ExternalInterface.call("ffish_run", "relayON");
			//fscommand("ffish_run", "relayON");
		}

		
		/**
		 * Turns off the sneeze relay by calling the
		 * relayOFF function within the SWFKit project
		 * 
		 * @param	e TimerEvent
		 */
		private function turnOff(e:TimerEvent):void
		{	
			myTimer.removeEventListener(TimerEvent.TIMER, turnOff);
			ExternalInterface.call("ffish_run", "relayOFF");
			//fscommand("ffish_run", "relayOFF");
			helper.log("Sneeze Relay Off - Engine timer");
		}
	}	
}