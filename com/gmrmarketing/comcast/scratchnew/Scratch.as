package com.gmrmarketing.comcast.scratchnew
{
	...
	public class Scratch extends EventDispatcher
	{
		...
		{	
			...
			intro = new scratchIntro();		
			intro.x = 55;
			intro.y = 800;// 165;
			intro.btnPlay.theText.text = "PLAY NOW";
			container.addChild(intro);
			TweenLite.to(intro, .5, { y:165 } );
		...
		private function begin(e:MouseEvent):void
		{
			...
			TweenLite.to(intro, .5, { y:800, onComplete:init } );			
		}
		...
		private function init():void
		{
			...
			circShadow = new DropShadowFilter(1, 0, 0, 1, 3, 3, 1, 2);
			...
			var circles:Bitmap = new Bitmap(new circs(1024, 768));
			...
			scratchImage2 = new espn(sqSize, sqSize);			
			scratchImage3 = new hgtv(sqSize, sqSize);
			scratchImage4 = new history(sqSize, sqSize);
		...
		{			
			...
			switch(whichClip) {
				case 2:
					animIcon = new iconESPN(); //library clip
					container.addChild(animIcon);
					animIcon.x = whichRect.left;
					animIcon.y = whichRect.top;
					TweenLite.to(animIcon, .75, { x:baseX + topIcons.coverESPN.x + 4, y:baseY + topIcons.coverESPN.y + 19, width:64, height:26, onComplete:killIcon } );
					TweenLite.to(topIcons.coverESPN, .5, { alpha:0, delay:.75 } );
					break;
				case 3:
					animIcon = new iconHGTV(); //library clip
					container.addChild(animIcon);
					animIcon.x = whichRect.left;
					animIcon.y = whichRect.top;
					TweenLite.to(animIcon, .75, { x:baseX + topIcons.coverHGTV.x + 4, y:baseY + topIcons.coverHGTV.y + 16, width:60, height:30, onComplete:killIcon } );
					TweenLite.to(topIcons.coverHGTV, .5, { alpha:0, delay:.75 } );
					break;
				case 4:
					animIcon = new iconHistory(); //library clip
					container.addChild(animIcon);
					animIcon.x = whichRect.left;
					animIcon.y = whichRect.top;
					TweenLite.to(animIcon, .75, { x:baseX + topIcons.coverHistory.x + 9, y:baseY + topIcons.coverHistory.y + 14, width:46, height:46, onComplete:killIcon } );
					TweenLite.to(topIcons.coverHistory, .5, { alpha:0, delay:.75 } );
					break;
			}
		}
		...
		private function killIcon():void
		{
			...
			if (topIcons.coverESPN.alpha == 0 && topIcons.coverHGTV.alpha == 0 && topIcons.coverHistory.alpha == 0) {
				moveMouse();
			}
		}
