package com.gmrmarketing.comcast.flex
{
	...
	import com.gmrmarketing.comcast.flex.HairDraw;
	...
	public class Main extends MovieClip
	{
		...
		{	
			...
			kiosk.attractInit(this.stage, 10000); //6 minutes
			kiosk.addEventListener(KioskEvent.START_ATTRACT, showAttract, false, 0, true);
			
			utility = new Utility(); //for the gridLoc method
			...
			player.autoSizeOff();
			player.setSmoothing();
			player.setVidSize( { width:1366, height:768 } )
		...
		/**
		 * Shows the Welcome to On Demand text for two seconds
		 */
		private function showIntro():void
		{
			introText = new welcome(); //lib clip	- Welcome to On Demand		
			addChild(introText);
			...
			if (whichKey == 79) {
				...
				if (attractClip) {
					if (contains(attractClip)) {
						...
						kiosk.attractStart(); //reenable attract loop testing
						showIntro();
					}
				}else{
		...
		{			
			...
			upperLeft.listDataHolder.theDescription.text = theData.description;
		...
		private function showAttract(e:KioskEvent):void
		{
			player.hideVideo();
			hideProgressBar();
			barker.pauseVideo();
			...
			if (attractClip == null) {
				...
				hd = new HairDraw(attractClip, 0, 0);
			}
			if (!contains(attractClip)) {
