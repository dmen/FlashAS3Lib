...
package  com.gmrmarketing.comcast.taylorswift.photobooth
{
	...
	{
		private const DATA_FILE_NAME:String = "xfinityTSLoftQueued.csv"; //current users / not yet uploaded
		private const SAVED_FILE_NAME:String = "xfinityTSLoftSaved.csv"; //users successfully uploaded
		...
		private function uploadNext():void
		{
			if (hubble.hasToken() && !hubble.isBusy() && users.length > 0) {			
				...
				hubble.submit(new Array(curUpload.email, curUpload.image));
			}
		}
