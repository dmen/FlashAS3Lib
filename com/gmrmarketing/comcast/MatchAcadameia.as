package com.gmrmarketing.comcast
{
	...
	public class MatchAcadameia extends MovieClip
	{
		//private var grid:screenGrid; //library clip
		 ...
		 * CONSTRUCTOR
		 */
		public function MatchAcadameia()
		{
			...
			*/
			...
			r4 = new reflect();
			r5 = new reflect();
			r1.constructReflection(grid.p4, .03, 0, 100, -16);
			r2.constructReflection(grid.p8, .03, 0, 100, 2);
			r3.constructReflection(grid.p12, .03, 0, 100, 10);
			r4.constructReflection(grid.p16, .03, 0, 100, -12);
			r5.constructReflection(grid.p20, .03, 0, 100, -40);
				*/		
		...
		private function createPaperSprites():void
		{
			...
			for (var i:int = 0; i < randomLogos.length; i++) {
				...
				this["p" + (i + 1)].holder.addChildAt(myPaperSprite,0);
			}
		}
		...
		{			
			...
			var cRef:Class;
			for (var i:int = 0; i < randomLogos.length; i++) {				
				myPaperSprite = this["p" + (i + 1)].holder.getChildAt(0);
		...
		{		
			...
			for (i = 0; i < randomLogos.length; i++) {
					...
					TweenLite.to(this["p" + (i + 1)].holder.getChildAt(0), 1, { rotationX:180 } );
		...
		{		
			...
			for (i = 0; i < randomLogos.length; i++) {
				if(i != 12){
					TweenLite.to(this["p" + (i + 1)].holder.getChildAt(0), 1, { rotationX:0 } );
				}
			}			
		}
