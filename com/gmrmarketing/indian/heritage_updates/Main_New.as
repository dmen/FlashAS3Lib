 ...
 * Updates: 11/2012
 * 		Remove Screen Saver
 * 		Remove Road Ahead from Hearth menu and admin menu
 */
package com.gmrmarketing.indian.heritage_updates
{
	...
	import com.gmrmarketing.indian.heritage_updates.Admin;
	import com.gmrmarketing.indian.heritage_updates.MainMenu;
	...
	public class Main_New extends MovieClip
	{
		...
		private var timeoutHelper:TimeoutHelper; //for returning to intro screen if app times out
		...
		public function Main_New()
		{
			...
			kbd.loadXML("indianKeyboard.xml");
			kbd.x = 285;
			kbd.y = 712;
		...
		{			
			...
			if (hearthMode) {
				...
				//showIntro(); //screen saver - not used
				removeIntro();
			}
		}
		...
		*/
		...
		private function removeIntro(e:MouseEvent = null):void
		{
			...
			if (hearthMode) {
				...
				//bottom nav re-added in loadSection if hearthmode is true
				while (numChildren) {
					removeChildAt(0);
				}
