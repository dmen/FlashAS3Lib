package com.gmrmarketing.altria.leadership
{
	...
	public class TTS extends MovieClip
	{
		...
		{
			curQuesID = 0;
			lastTime = "2011-05-27";//initial start time - only works if setCurrentTime() is commented out in questionLoaded()
			//comments prior to this time won't be loaded
			...
			displayPoints = new Array(new Point(310,275));			
			...
			btnClear.addEventListener(MouseEvent.CLICK, clearAllReplies, false, 0, true);
			
			//quesTimer.start();
			cullingTimer.start();
			//setCurrentTime(); //replies before 'now' won't be loaded - comment out to load all replies
			checkForResults();
		}
		...
		*/
		...
		{			
			...
			for (var i:int = 0; i < c; i++) {
				...
				var al:Number = (20 + Math.random() * 35) / 100; // .2 - .55
				...
				b.balloon.height = b.theText.textHeight + 30;
				b.bott.y = b.theText.textHeight + 28;
				b.scaleX = b.scaleY = scal;
				b.alpha = al;
				b.x = p.x;
				b.y = p.y;
				b.filters = [blur];
				TweenMax.to(b.balloon, 0, { colorMatrixFilter: { saturation:0 }} );
				TweenMax.to(b.bott, 0, { colorMatrixFilter: { saturation:0 }} );
		...
		private function clearAllReplies(e:MouseEvent = null):void
		{	
			cullingTimer.reset();			
			var inc:Number = 0;
		...
		{			
			if (replies.length > 0 && !isViewing) {
				...
				TweenMax.to(rep.bott, .5, { colorMatrixFilter: { saturation:1 }} );
				TweenMax.to(rep.balloon, .5, { colorMatrixFilter: { saturation:1 }} );				
			}
		}
		...
		{
			TweenMax.to(rep, .5, { delay:3, alpha:oldAlpha, scaleX:oldScaleX, scaleY:oldScaleY, blurFilter: { blurX:0, blurY:0 }, onComplete:restartQuoteMotion, onCompleteParams:[rep] } );
			TweenMax.to(rep.bott, .5, { delay:3, colorMatrixFilter: { saturation:0 }} );
			TweenMax.to(rep.balloon, .5, { delay:3, colorMatrixFilter: { saturation:0 }} );	
		}
