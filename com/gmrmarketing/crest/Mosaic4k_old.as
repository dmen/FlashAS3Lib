package com.gmrmarketing.crest
{
	...
	{				
		...
		private const SERVER_TILE_WIDTH:int = 40; //loaded tiles are 30x30
		...
		private var tileWidth:int = 6;
		private var tileHeight:int = 4;
		...
		private var tileXML:XML = new XML();
		...
		{				
			...
			mosaic = new mosaicBmd(600, 400); //library image			
			
			//empty, black image on stage
			targetImage = new BitmapData(4000, 3000, false, 0x000000);
			target = new Bitmap(targetImage);			
			addChild(target);			
			target.scaleX = .15; //to fit to 600 x 400
			target.scaleY = .134;
