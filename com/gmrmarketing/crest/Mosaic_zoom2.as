package com.gmrmarketing.crest
{
	...
	public class Mosaic_zoom2 extends EventDispatcher
	{				
		private var tiles:Array;
		private var tileLoader:Loader;				
		private var container:Sprite;
		
		private const SERVER_TILE_WIDTH:int = 50; //size of individual tiles in the catalog
		private const SERVER_TILE_HEIGHT:int = 36;
		
		private const TOTAL_TILES:int = 20000; //minimum number of tiles to use - if there are less tiles in the
		...
		private var botSprite:Sprite;
		
		
		
		public function Mosaic_zoom2($container:Sprite) 
		{
			container = $container;
		...
		private function tilesLoaded(e:Event):void
		{
			...
			var currentArea:int = SERVER_TILE_WIDTH * SERVER_TILE_HEIGHT; //currently 50 x 36 = 1800
			trace("currentArea", currentArea);
			//targetPixels calculated in constructor - total pixels in the mosaic target
			var targetArea:Number = targetPixels / tt; //mosaic target pixels per tile - for getting average - 24
			trace("target area", targetArea);
			var ratio:Number = Math.sqrt(targetArea / currentArea);			

			//size of individual tiles in the mosaic for getting average from
			tileWidth = Math.round(SERVER_TILE_WIDTH * ratio);
			tileHeight = Math.round(SERVER_TILE_HEIGHT * ratio);
			...
			trace("tiles per row", tilesPerRow);
			trace("tiles per col", tilesPerCol);
		...
		{				
			...
			if(tileIndex < 8000){
				topSprite.addChild(temp);
			}else {
				botSprite.addChild(temp);
			}
			temp.x = imX;
			temp.y = imY;
